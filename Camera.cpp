...
Camera::Camera(glm::vec3 eye, glm::vec3 forward, glm::vec3 up, double fov, double focalLength, int width, int height)
        : eye(eye), forward(forward), up(up),
          fov(fov), focalLength(focalLength), width(width), height(height) {
    aspectRatio = width / (float) height;
    auto center = eye + forward;
    V = glm::lookAt(eye, center, up);
    NormalMatrix = glm::transpose(glm::inverse(V));
}
...
glm::dvec2 Camera::rasterToCamera(glm::dvec2 rasterCoord) const {
    ...
    glm::dvec2 screenCoord = glm::vec2(((rasterCoord.x / width) * 2) - 1,
                                       (((height - rasterCoord.y) / height) * 2) - 1);
...
glm::dvec3 Camera::normalToCamera(glm::dvec3 normal) {
    return (NormalMatrix * glm::dvec4(normal, 1));
}