...
public:
    ...
    Plane(glm::dvec3 p0, glm::dvec3 normal, Material material) : p0(p0), normal(normal), material(material) {}
...
public:
    ...
    bool hit(const Ray &r, HitInfo &info) override {
        ...
        if (abs(denom) > 1e-6) {
            ...
            if (info.t < 0) return false;
            info.point = r.origin + r.direction * info.t;
            if (info.point.x > max.x || info.point.y > max.y || info.point.z > max.z || info.point.x < min.x || info.point.y < min.y || info.point.z < min.z) return false;
            info.normal = denom > 0 ? normal : -normal;
            info.material = material;
            return true;
        }
...
public:
    ...
    Triangle(glm::dvec3 v0, glm::dvec3 v1, glm::dvec3 v2, Material material) : v0(v0), v1(v1), v2(v2), material(material) {}
